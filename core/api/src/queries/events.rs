...
pub async fn batch_insert_events(pool: &PgPool, events: &[BatchEvent]) -> Result<u64, sqlx::Error> {
    if events.is_empty() {
        return Ok(0);
    }

    let mut query = String::from("INSERT INTO events (account_id, event_type, payload) VALUES ");
...
pub async fn batch_delete_old_events(pool: &PgPool, hours_old: i32) -> Result<u64, sqlx::Error> {
    let result =
        sqlx::query("DELETE FROM events WHERE created_at < now() - INTERVAL '1 hour' * $1")
            .bind(hours_old)
            .execute(pool)
            .await?;
