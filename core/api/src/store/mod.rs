...
pub async fn list_extensions(
    ...
    match list_store_extensions(&pool, filter, sort, pagination).await {
        Ok((rows, total)) => {
            ...
            (StatusCode::OK, Json(ExtensionListResponse {
                extensions,
                total,
                page: params.page,
...
pub async fn get_extension(
    ...
    match get_store_extension(&pool, &slug).await {
        Ok(Some(row)) => {
            ...
            (StatusCode::OK, Json(ExtensionDetailResponse {
                slug: row.slug,
                name: row.name,
                version: row.version,
                description: row.description,
...
pub async fn get_extension_manifest(
    ...
    match get_store_extension(&pool, &slug).await {
        Ok(Some(row)) => {
            (StatusCode::OK, Json(row.manifest)).into_response()
        }
        Ok(None) => {
            (StatusCode::NOT_FOUND, Json(serde_json::json!({
                "error": "Extension not found"
...
pub async fn list_categories(
    ...
    match get_extension_categories(&pool).await {
        Ok(rows) => {
            let categories: Vec<CategoryInfo> = rows.into_iter().map(|(slug, count)| {
                CategoryInfo {
